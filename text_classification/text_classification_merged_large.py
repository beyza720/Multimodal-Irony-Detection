...
CONFIGURATIONS = [
    {'batch_size': 2, 'learning_rate': 5e-5},
...
parser.add_argument('--wandb_proj_name', default="mmsd_merged_large", type=str, help='wandb project name')
parser.add_argument('--num_epochs', default=3, type=int, help='number of epochs (reduced for large model)')
...
parser.add_argument('--output_dir', default="sweep_merged_large", type=str, help='output directory')
...
def cleanup_gpu():
    """Clean up GPU memory aggressively for large model"""
...
def load_data():
    ...
    logger.info("Loading merged training data for Large model...")
    try:
        ...
        max_text_length = 400  
        train_df['text'] = train_df['text'].astype(str).str[:max_text_length]
        valid_df['text'] = valid_df['text'].astype(str).str[:max_text_length]
        test_df['text'] = test_df['text'].astype(str).str[:max_text_length]
        logger.info(f"Truncated text to max {max_text_length} characters for memory efficiency")
...
def init_model(config=None):
    try:
        with wandb.init(config=config, project=wandb_proj_name) as run:
            config = wandb.config
            run_name = f"merged_large|model:{model_name}|batch:{config.batch_size}|lr:{config.learning_rate}"
            params = {
...
def train(params):
    ...
    try:
        logger.info(f"Starting Large model training with batch size: {params['batch_size']}, learning rate: {params['lr']}")
        
        cleanup_gpu()
        
        logger.info("Loading XLM-RoBERTa Large model...")
        ...
        logger.info("Configuring training arguments for Large model...")
        training_args = TrainingArguments(
            ...
            per_device_eval_batch_size=max(1, params["batch_size"] // 2),  
        ...
        logger.info("Initializing trainer for Large model...")
        trainer = Trainer(
            model=model,
            args=training_args,
        ...
        trainer.save_model()
        logger.info("Evaluating Large model...")
...
def evaluate(model, run_name):
    ...
    try:
        logger.info("Starting Large model evaluation...")
        
        test_dataset = Dataset.from_pandas(test_df)
        logger.info("Tokenizing test dataset...")
        tokenized_test = test_dataset.map(
            lambda x: tokenizer(x["text"], truncation=True, max_length=256, padding=True),
            batched=True,
            remove_columns=["text"],
            batch_size=1000  
        )
        ...
        logger.info(f"Large model evaluation results - F1-macro: {metrics['f1-macro']:.4f}, Accuracy: {metrics['accuracy']:.4f}")
...
logger.info("Loading XLM-RoBERTa Large tokenizer...")
...
if __name__ == "__main__":
    try:
        logger.info("Starting main execution for merged datasets with Large model...")
        ...
        if args.test_only:
            ...
            print("\nTest Results:")
            print(result_df)
            result_df.to_csv(output_dir / f"test_results_merged_large_{model_name}.csv", sep="\t", encoding="utf-8")
            ...
            if args.config_index is not None:
                ...
                logger.info(f"Running Large model configuration {args.config_index}: {config}")
                init_model(config)
            else:
                logger.info("Running all Large model configurations...")
                for config in CONFIGURATIONS:
                    init_model(config)
            
            result_df = pd.DataFrame.from_dict(result_dict)
            print(result_df)
            result_df.to_csv(output_dir / f"sweep_merged_large_{model_name}.csv", sep="\t", encoding="utf-8", mode='a', header=False)
